...
my $TEMPFILE="${main::SCRIPT_TEMP}/backup_stop_$$.txt";

##--- Connect and issue command
my $cmd="${main::PGBIN}/psql -c \"SELECT pg_stop_backup()\" > ${TEMPFILE}";
system $cmd;
...
if ($line =~ /^\s*pg_stop_backup\s*$/) {$goodlines++;}
...
{
  $subject="${main::SERVERNAME} Postgres - backup_stop failure";
